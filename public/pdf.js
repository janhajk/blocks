...
(function() {
      ...
      let output = function(block, next) {
            let self = block;
            // Render DOM of block
            // self.render();
            // if top block, then clear content area
            ParseHtml(content, self.data.content);

            async.eachLimit(self.data.children, 1, function(childBlock, callback) {
      ...
      function ParseHtml(cnt, htmlText) {
            var html = $.parseHTML(htmlText.replace(/\t/g, "").replace(/\n/g, ""));
      ...
      Block.fn.pdf = function() {
            content = [];
            let self = this;
            output(self, function() {
                  let title = $.parseHTML(self.data.content);
                  title = title.map(x => $(x).text());
                  let docDefinition = {
      ...
      Block.fn.blockPanelMenu.push(function(block) {
            return {
                  title: 'PDF',
                  icon: 'export',
                  action: function() {
