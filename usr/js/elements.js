...
element.prototype.toTrash = function() {
  ...
  id   = this.cid,
  data = l.data.data,
  trashTree = function(cid) {
    var c;
    for (c in data[cid].children[0]) {
      trashTree(data[cid].children[0][c]);
    }
    for (c in data[cid].children[1]) {
      trashTree(data[cid].children[1][c]);
    }
    delete(l.data.data[cid]);
  },
  options = {
    action : 'trash',
    id     : id
...
element.prototype.toggleActive = function() {
  ...
  toggleAll = function(cid) {
    var c;
    data[cid].active = state;
    for (c in data[cid].children[0]) {
      toggleAll(data[cid].children[0][c]);
    }
    for (c in data[cid].children[1]) {
      toggleAll(data[cid].children[1][c]);
    }
  },
  options = {
    action : 'toggleactive',
...
element.prototype.renderHistory = function() {
  ...
  for (i in this.revisions) {
    ...
    items.push(render(this.cid+'-'+i, content));
  } 
  return items;
};
...
element.prototype.render = function(level) {
  ...
  return '<li id="listeItem-' + this.cid + '" class="cItem" level="'+level+'">' + this.renderContent() + '</li>';
};
...
element.prototype.renderContent = function() {
  return '<div id="sfield-'+this.cid+'" class="sfield" style="background-color:">'+
    '<div id="scontent-'+this.cid+'" class="scontent">'+this.formatForOutput()+'</div>'+
    '<div class="newline"></div>'+
  '</div>';        
};
